...
def crawMorningFinanceDailyNews(linkUrl):
    ...
    for mainContext in contextList:
        ...
        currentArray.append([str(uuid.uuid1()),linkUrl,imageUrl,title,pubDate,descriptContext,'CHINA','FXNET'])
    return currentArray

def writeMorningFinanceDailyNews():
    link = 'http://news.fx168.com/politics/cn/'
    currentArray = crawMorningFinanceDailyNews(link)
    dbManager = CommonsMysqlUtils._dbManager
    SQL = " DELETE  FROM  MORNING_FINANCENEWS_RESOURCE_TABLE  WHERE  SOURCEFLAG = 'FXNET' " \
          " AND  NEWSFLAG='CHINA' "
