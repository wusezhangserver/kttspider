...
def writeDailySinaBlog():
    ...
    cursor = conn.cursor()
    cursor.execute("DELETE FROM  DAILYBLOG_RESOURCE_DETAIL_TABLE")
    conn.commit()
    for row in SinaBlogSpiderUtils.returnAuthorList():
        currentReult = dailySinaBlog(row[0],row[1])
        cursor.executemany('INSERT  INTO  DAILYBLOG_RESOURCE_DETAIL_TABLE (ID,TITLE,LINKURL,PUBDATE,IMAGEURL) VALUES (%s,%s,%s,%s,%s)',currentReult)
        conn.commit()
    cursor.close()
    conn.close()
